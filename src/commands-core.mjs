...
export async function initCommand(args, { cwd }) {
  ...
  const files = [
    ...
    [paths.engagement, `${JSON.stringify(engagementTemplate(now, expiry), null, 2)}\n`],
...
export async function doctorCommand(args, context) {
  ...
  try { await loadConfig(context.cwd, { userPaths: context.users }); checks.push({ name: "config", ok: true, detail: "loaded" }); }
  catch (error) { checks.push({ name: "config", ok: false, detail: error.message }); }
  try { const engagement = await loadEngagement(context.cwd); checks.push({ name: "engagement", ok: true, detail: engagement.id }); }
  catch (error) { checks.push({ name: "engagement", ok: false, detail: error.message }); }
  try { const mcp = await loadMcpConfig(context.cwd, { userPaths: context.users, env: context.env }); checks.push({ name: "mcp", ok: true, detail: `${Object.keys(mcp.servers).length} server(s)` }); }
  catch (error) { checks.push({ name: "mcp", ok: false, detail: error.message }); }
...
export async function pathsCommand(args, context) {
  const payload = { user: context.users, project: projectPaths(context.cwd) };
  if (args.includes("--json")) printJson(payload); else printTable(Object.entries(payload.user).map(([name, detail]) => ({ name, detail })));
  return 0;
}
...
export function profilesCommand(args) {
  if (args.includes("--json")) printJson(PROFILES); else printTable(PROFILES.map((item) => ({ name: item.name, capabilities: item.capabilities.join(", "), purpose: item.purpose })));
  return 0;
}
...
export async function mcpCommand(args, context) {
  ...
  try {
    if (action === "status") {
      const rows = runtime.manager.servers().map((server) => ({ name: server.name, transport: server.transport, enabled: server.enabled !== false, endpoint: server.command ?? server.url }));
    ...
    if (action === "tools") {
      if (serverName) {
        ...
        const rows = [];
        for (const server of runtime.manager.servers().filter((item) => item.enabled !== false)) {
          for (const tool of await runtime.manager.tools(server.name)) rows.push({ server: server.name, name: tool.name, risk: tool.classification.risk, trusted: tool.classification.trusted });
        }
        if (args.includes("--json")) printJson(rows); else printTable(rows);
      }
      return 0;
...
export async function approvalsCommand(args, context) {
  ...
  if (action === "grant") {
    const approval = await store.grant({
      risk: flag(args, "--risk") ?? "active",
      ttlMinutes: Number(flag(args, "--ttl") ?? 20),
