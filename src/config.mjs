...
export async function loadMcpConfig(cwd = process.cwd(), options = {}) {
  ...
  const merged = {
    schemaVersion: 1,
    servers: { ...(user.servers ?? {}), ...(local.servers ?? {}) }
  };
  const interpolated = interpolate(merged, options.env ?? process.env);
  validateMcpConfig(interpolated);
  return interpolated;
}

async function readTomlIfPresent(path) {
  try { return parseToml(await readFile(path, "utf8")); }
  catch (error) {
    if (error?.code === "ENOENT") return {};
    throw new Error(`Invalid configuration ${path}: ${error.message}`);
  }
}
...
function deepMerge(base, override) {
  ...
  for (const [key, value] of Object.entries(override)) {
    if (isObject(value) && isObject(base[key])) base[key] = deepMerge({ ...base[key] }, value);
    else base[key] = value;
  }
  return base;
}
...
function validateConfig(config) {
  if (!isObject(config.model) || !["mock", "openai-compatible"].includes(config.model.provider)) {
    throw new Error("model.provider must be mock or openai-compatible");
  }
...
function validateEngagement(value) {
  ...
  for (const key of ["id", "name", "authorizedBy", "authorizationReference", "startsAt", "expiresAt", "mode"]) {
    if (typeof value[key] !== "string" || !value[key].trim()) throw new Error(`Engagement ${key} is required`);
  }
...
function interpolate(value, env) {
  ...
  if (isObject(value)) return Object.fromEntries(Object.entries(value).map(([key, entry]) => [key, interpolate(entry, env)]));
  return value;
}
