...
export class StdioTransport extends EventEmitter implements McpTransport {
  ...
  public constructor(private readonly config: McpServerConfig) { super(); }
  ...
  public async start(): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        if (!settled) { settled = true; reject(new Error("MCP process startup timed out")); }
      ...
      this.child.once("spawn", () => { if (!settled) { settled = true; clearTimeout(timer); this.started = true; resolve(); } });
      this.child.once("error", (error: Error) => { if (!settled) { settled = true; clearTimeout(timer); reject(error); } });
      this.child.on("exit", (code: number | null) => {
        this.started = false;
        for (const pending of this.pending.values()) { clearTimeout(pending.timer); pending.reject(new Error(`MCP process exited with code ${code}`)); }
        this.pending.clear();
      });
    });
  }
  ...
  private consume(chunk: string): void {
    ...
    while ((newline = this.buffer.indexOf("\n")) >= 0) {
      ...
      } catch { this.emit("stderr", `Invalid MCP JSON received: ${line}\n`); }
    }
  }
...
export class HttpTransport implements McpTransport {
  ...
  public async start(): Promise<void> { if (!this.config.url) throw new Error("HTTP MCP server requires url"); this.started = true; }

  private async send(payload: unknown): Promise<any> {
    ...
    try {
      ...
      const message = contentType.includes("text/event-stream")
        ? JSON.parse(text.split(/\r?\n/).filter((line: string) => line.startsWith("data:")).map((line: string) => line.slice(5).trim()).at(-1) ?? "{}")
        : JSON.parse(text);
  ...
  public async request(method: string, params: unknown = {}): Promise<any> { return this.send({ jsonrpc: "2.0", id: ++this.sequence, method, params }); }
  public async notify(method: string, params: unknown = {}): Promise<void> { await this.send({ jsonrpc: "2.0", method, params }); }
  public async close(): Promise<void> { this.started = false; }
}
