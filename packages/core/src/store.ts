...
export class EngagementStore {
  ...
  public constructor(
    cwd = process.cwd(),
    private readonly redactSecrets = true,
    private readonly maxEvidenceBytes = 1_048_576
  ) {
    this.root = path.join(cwd, ".webcat");
  }

  public async init(): Promise<void> {
    for (const directory of ["state", "evidence", "reports"]) {
      await fs.mkdir(path.join(this.root, directory), { recursive: true });
    }
  }

  private sanitize(value: unknown): unknown {
    return this.redactSecrets ? redactValue(value) : value;
  }
  ...
  public async appendAudit(
    ...
    const existing = await this.listAudit();
    ...
    const unsigned = {
      ...
      actor,
      status,
      details: this.sanitize(details) as Record<string, unknown>,
      ...(previousHash ? { previousHash } : {})
    };
    const record: AuditRecord = { ...unsigned, hash: sha256(JSON.stringify(unsigned)) };
    await this.appendJsonl("audit.jsonl", record);
    return record;
  }

  public async listAudit(): Promise<AuditRecord[]> {
    return this.readJsonl<AuditRecord>("audit.jsonl");
  }

  public async verifyAuditChain(): Promise<{ valid: boolean; invalidRecordId?: string }> {
    const records = await this.listAudit();
    let previousHash: string | undefined;
    for (const record of records) {
      const { hash, ...unsigned } = record;
      if (unsigned.previousHash !== previousHash || sha256(JSON.stringify(unsigned)) !== hash) {
        return { valid: false, invalidRecordId: record.id };
      }
      previousHash = hash;
    }
    return { valid: true };
  ...
  public async addEvidence(
    ...
    if (Buffer.byteLength(body) > this.maxEvidenceBytes) {
      body = JSON.stringify({
        ...
        preview: body.slice(0, this.maxEvidenceBytes)
      });
    }
    const record: EvidenceRecord = {
      ...
      agent,
      kind,
      summary: String(this.sanitize(summary)),
      data: JSON.parse(body) as unknown,
      sha256: sha256(body),
      byteLength: Buffer.byteLength(body),
      parentEvidenceIds: [...new Set(parentEvidenceIds)]
    };
    await fs.writeFile(path.join(this.root, "evidence", `${record.id}.json`), JSON.stringify(record, null, 2), { mode: 0o600 });
    const { data: _data, ...indexRecord } = record;
    await this.appendJsonl("evidence.jsonl", indexRecord);
    return record;
  }

  public async listEvidence(): Promise<Array<Omit<EvidenceRecord, "data">>> {
    return this.readJsonl<Omit<EvidenceRecord, "data">>("evidence.jsonl");
  }
  ...
  public async listHypotheses(): Promise<Hypothesis[]> {
    return this.readJsonl<Hypothesis>("hypotheses.jsonl");
  }
  ...
  private findingFingerprint(input: Pick<Finding, "title" | "target" | "description">): string {
    return sha256(`${input.title.trim().toLowerCase()}\n${input.target?.trim().toLowerCase() ?? ""}\n${input.description.trim().toLowerCase()}`);
  }

  public async addFinding(
    input: Omit<Finding, "id" | "fingerprint" | "createdAt" | "updatedAt">
  ): Promise<Finding> {
    const fingerprint = this.findingFingerprint(input);
    const existing = (await this.listFindings()).find((item) => item.fingerprint === fingerprint && item.status !== "rejected");
    if (existing) return existing;
    const now = new Date().toISOString();
    const record: Finding = { ...input, id: makeId("find"), fingerprint, createdAt: now, updatedAt: now };
    await this.appendJsonl("findings.jsonl", record);
    return record;
  }

  public async listFindings(): Promise<Finding[]> {
    return this.readJsonl<Finding>("findings.jsonl");
  }
  ...
  public async listApprovals(): Promise<ApprovalRecord[]> {
    ...
    const now = Date.now();
    return values.map((value) => value.status === "active" && new Date(value.expiresAt).getTime() <= now
  ...
  public async saveSession(session: SessionRecord): Promise<void> {
    ...
    await fs.writeFile(path.join(this.root, "state", `${session.id}.json`), JSON.stringify(updated, null, 2), { mode: 0o600 });
    await fs.writeFile(path.join(this.root, "state", "current.json"), JSON.stringify(updated, null, 2), { mode: 0o600 });
  }
  ...
  public async loadSession(id = "current"): Promise<SessionRecord | undefined> {
    try {
      return JSON.parse(await fs.readFile(path.join(this.root, "state", `${path.basename(id)}.json`), "utf8")) as SessionRecord;
    } catch {
      return undefined;
    }
  }
  ...
  public async listSessions(): Promise<SessionRecord[]> {
    ...
    for (const entry of await fs.readdir(path.join(this.root, "state"))) {
      if (!entry.endsWith(".json") || ["current.json", "workflow.json"].includes(entry)) continue;
      try {
        output.push(JSON.parse(await fs.readFile(path.join(this.root, "state", entry), "utf8")) as SessionRecord);
      } catch {
        // Ignore malformed session files and leave them for operator inspection.
      }
    }
  ...
  public async saveState(state: WorkflowState, extra: Record<string, unknown> = {}): Promise<void> {
    await this.init();
    await fs.writeFile(
      path.join(this.root, "state", "workflow.json"),
      JSON.stringify({ state, updatedAt: new Date().toISOString(), ...(this.sanitize(extra) as object) }, null, 2),
      { mode: 0o600 }
    );
  }
}
